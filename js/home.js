...
(function() {
    ...
    // ========== 主页绑定会话存储路由 ==========
    // 开关状态始终全局存储（不随会话变化）
    let homeSessionBindEnabled = true;
    localStorage.setItem('home_session_bind', 'true');
    ...
    function homeGetItem(key) {
        const routed = localStorage.getItem(homeKey(key));
        if (routed !== null) return routed;
        return homeSessionBindEnabled ? localStorage.getItem(key) : null;
    }
    ...
    async function homeGetLargeItem(key) {
        ...
        if (typeof localforage !== 'undefined') {
            ...
            if (homeSessionBindEnabled) {
                const globalVal = await localforage.getItem(key);
                if (globalVal !== null) return globalVal;
            }
        }
        const routed = localStorage.getItem(k);
        if (routed !== null) return routed;
        return homeSessionBindEnabled ? localStorage.getItem(key) : null;
    }
    ...
    window.handleAvatarUpload = function(event) {
        ...
        reader.onload = function(e) {
            ...
            homeSetItem(`home_avatar_${currentAvatarTarget}`, url);
            homeSetItem(`profile_${currentAvatarTarget}`, JSON.stringify(profileData[currentAvatarTarget]));
            window.dispatchEvent(new CustomEvent('homeGlobalUpdated', { detail: { key: `home_avatar_${currentAvatarTarget}`, value: url } }));
            window.dispatchEvent(new CustomEvent('homeGlobalUpdated', { detail: { key: `profile_${currentAvatarTarget}`, value: JSON.stringify(profileData[currentAvatarTarget]) } }));
    ...
    window.saveProfileEdit = function() {
        ...
        window.dispatchEvent(new CustomEvent('homeGlobalUpdated', { detail: { key: `profile_${currentProfileTarget}`, value: JSON.stringify(data) } }));

        if (currentProfileTarget === 'partner' && window.SESSION_ID && Array.isArray(window.sessionList) && typeof localforage !== 'undefined') {
            const currentSession = window.sessionList.find(s => s.id === window.SESSION_ID);
            if (currentSession) {
                currentSession.name = name;
                const appPrefix = typeof APP_PREFIX !== 'undefined' ? APP_PREFIX : 'zcard_';
                localforage.setItem(`${appPrefix}sessionList`, window.sessionList).catch(() => {});
            }
        }
    ...
    async function loadSavedSettings() {
        ...
        ['me', 'partner'].forEach(who => {
            ...
            // 头像和昵称：优先从聊天当前会话设置同步，没有则从 Home 当前会话读取
            if (window.settings) {
                ...
                if (settingAvatar) {
                    ...
                    const savedAvatar = homeGetItem(`home_avatar_${who}`);
    ...
    window.updateHomeAvatar = function(who, url) {
        ...
        // 更新当前会话数据，切换对象后各自恢复自己的头像和资料
        profileData[who].avatar = url;
        homeSetItem(`home_avatar_${who}`, url);
        homeSetItem(`profile_${who}`, JSON.stringify(profileData[who]));
        window.dispatchEvent(new CustomEvent('homeGlobalUpdated', { detail: { key: `home_avatar_${who}`, value: url } }));
        window.dispatchEvent(new CustomEvent('homeGlobalUpdated', { detail: { key: `profile_${who}`, value: JSON.stringify(profileData[who]) } }));
    ...
    function getAppPrefix() {
        return typeof APP_PREFIX !== 'undefined' ? APP_PREFIX : 'zcard_';
    }

    async function getLatestSessionList() {
        let list = Array.isArray(window.sessionList) ? window.sessionList : [];
        if (typeof localforage !== 'undefined') {
            try {
                const stored = await localforage.getItem(`${getAppPrefix()}sessionList`);
                if (Array.isArray(stored)) {
                    list = stored;
                    window.sessionList = stored;
                    if (typeof sessionList !== 'undefined') sessionList = stored;
                }
            } catch(e) {}
        }
        return list;
    ...
    async function createObjectSessionAndSwitch() {
        ...
        const newSession = {
            id: newId,
            name: `对象 ${new Date().toLocaleDateString()}`,
            createdAt: Date.now()
        };
        const list = await getLatestSessionList();
    ...
    // 会话切换弹窗
    window.showSessionSwitcher = async function() {
        const sessionList = await getLatestSessionList();
        ...
        panel.innerHTML = `
            ...
            <div style="display: flex; flex-direction: column; gap: 8px; max-height: 50vh; overflow-y: auto;">
                ...
                `).join('') : '<div style="padding:16px;text-align:center;color:var(--text-secondary);font-size:13px;">还没有可切换对象，请先新建对象</div>'}
            </div>
            <div style="margin-top: 14px; display: flex; gap: 10px;">
                ...
                <button id="session-switch-new" style="flex: 1; padding: 11px; border: none; border-radius: 12px; background: var(--accent-color); color: #fff; font-size: 13px; font-weight: 700; cursor: pointer; font-family: var(--font-family);">新建对象</button>
            </div>
        `;
        ...
        // 新建会话按钮
        panel.querySelector('#session-switch-new').onclick = async () => {
            overlay.remove();
    ...
    document.addEventListener('DOMContentLoaded', function() {
        const btn = document.getElementById('hero-session-switch-btn');
        if (btn) {
            btn.onclick = function(e) {
                e.preventDefault();
                e.stopPropagation();
                if (typeof window.showSessionSwitcher === 'function') window.showSessionSwitcher();
                return false;
            };
        }
    });
