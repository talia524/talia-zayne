...
function createMessageFragment(msg, prevMsg, nextMsg, lastSenderRef) {
    ...
    if (isRedPacket) {
        ...
        const mediaUrl = escapeMediaText(mediaRawUrl);
...
const addMessage = (message) => {
    ...
    if (message.sender && message.sender !== 'user' && message.sender !== 'system' && message.type !== 'system' && message.type !== 'call-event') {
        ...
        function sendMessage(textOverride = null, type = 'normal') {
            ...
            // 音频/视频文件由 media-chat.js 处理，sendMessage 不再发图片消息
            if (imageFile && imageFile.type && (imageFile.type.startsWith('audio/') || imageFile.type.startsWith('video/'))) {
                return;
            }
            ...
            if (imageFile && imageFile.size > MAX_IMAGE_SIZE) {
                showNotification('图片较大，正在尝试压缩后发送', 'info', 1500);
                // 不再阻止发送，optimizeImage 会自动压缩
            }
