    ...
    function shareProduct() {
        ...
        if (typeof addMessage === 'function') {
            addMessage({
                ...
                status: 'sent',
                type: 'share',
                shareData: { name: product.name, price: product.price, icon: product.icon, img: product.img, tag: '好物分享', tagColor: '#ff4757' }
            });
        }
    ...
    async function askTAPay() {
        ...
        if (typeof addMessage === 'function') {
            addMessage({
                ...
                status: 'sent',
                type: 'pay-request',
                shareData: { name: product.name, price: product.price, total: total, qty: state.modalQty, icon: product.icon, img: product.img, desc: '帮我买这个好不好~', tag: '💝 已帮TA付', tagColor: 'linear-gradient(135deg,#ff9a56,#ff6b35)' }
            });
        }
            ...
            setTimeout(() => {
                ...
                if (typeof addMessage === 'function') {
                    addMessage({
                        ...
                        status: 'received',
                        type: 'share',
                        shareData: { name: product.name, price: product.price, icon: product.icon, img: product.img, tag: '已代付', tagColor: '#ff9a56' }
                    });
                }
            }, 800);
        }, 1500 + Math.random() * 2000);
    ...
    async function tryAutoBuyFromCart() {
        ...
        if (typeof addMessage === 'function') {
            addMessage({
                ...
                status: 'received',
                type: 'share',
                shareData: { name: product.name, price: product.price, icon: product.icon, img: product.img, tag: '已自动下单', tagColor: '#ff9a56' }
            });
        }
    }
